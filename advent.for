...
1001	CONTINUE
	...
	OPEN(UNIT=1,FILE='ADVENT',ACCESS='SEQIN')
	SETUP=1
	LINUSE=1
	TRVS=1
	CLSSES=1
...
C  THE INDEX OF THE LAST NON-BLANK CHAR IN CHARS IS RETURNED IN LENG.
	...
	DATA MASK,BLANK/O"774000000000",' '/
...
50	M=1
	CALL MSPEAK(23)
CBP
C55	ACCEPT 56,(MSG(I),I=M+1,M+14),K
...
10	IDIST=-DIST
	DO 11 I=1,IDIST
	J=0
	IF(SHIFT.LT.0)J=O"200000000000"
11	SHIFT=((SHIFT.AND.INT(O"377777777777"))/2)+J
20	RETURN
30	DO 31 I=1,DIST
	J=0
	IF((SHIFT.AND.O"200000000000").NE.0)J=O"400000000000"
31	SHIFT=(SHIFT.AND.O"177777777777")*2+J
	RETURN
	END
...
C  FUNCTION I2 TAKES 2-DIGIT ASCII AND YIELDS DECIMAL VALUE.
	...
	D=I2(DAT(1))-1
	X=((SHIFT(DAT(1),14).OR.SHIFT(DAT(2),-21)).AND..NOT.O"1004020001")
	1	.OR.'-@@@-'
